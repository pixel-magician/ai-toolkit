...
const GpuMonitor: React.FC = () => {
  ...
  const content = useMemo(() => {
    ...
    if (!gpuData) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">无可用的GPU数据。</span>
        </div>
      );
    }
    ...
    if (!gpuData.hasNvidiaSmi) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <strong className="font-bold">未检测到NVIDIA GPU!</strong>
          <span className="block sm:inline"> 此系统上无法使用 nvidia-smi。</span>
    ...
    if (gpuData.gpus.length === 0) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">未找到GPU，但 nvidia-smi 可用。</span>
        </div>
      );
    }
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="text-xs text-gray-500">最后更新时间: {lastUpdated?.toLocaleTimeString()}</div>
      </div>
      {content}
    </div>
  );
